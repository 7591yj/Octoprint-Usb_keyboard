...
from octoprint.events import eventManager, all_events
...
class Usb_keyboardPlugin(octoprint.plugin.StartupPlugin,
	...
	def _key_event(self, event, payload):
		...
		if self._active_listening:
			if time.time() - self._active_listening_start > 10 * 60:
				...
				self._plugin_manager.send_plugin_message(self._identifier, {"key": key, "key_state": key_state,
																			"reply": "active_listening"})

		if key_state == "pressed":
			self.last_key_pressed = key
			self._logger.info("PRESSED: " + self.last_key_pressed)
		...
		for current_action in state_key_actions:
					...
					if command == "cancel_print":
						...
						if system_command is not None:
							...
							try:
								...
								returncode, stdout, stderr = self.caller.call(system_command,
																			  shell=should_use_separate_shell)
				...
				if key_values:
					for variable in self.listening_variables:
						...
						if variable_value is not None:
							self._logger.debug(
								f"Found variable value for key '{key}'. Setting variable '{variable}' as value '{variable_value}'.")
			...
			if current_action_type == "printer":
				...
				if can_send_commands and gcode_commands:
					...
					subbed_gcode_commands = [self.variable_sub(gcode.get("code", "")) for gcode in
											 filtered_gcode_commands]
					...
					if psucontrol_info:
						...
						if not psu_is_on and desired_state > -1:
							...
							if not tools_too_hot_to_turn_off_psu:
								...
								self._logger.debug(
									f"Found psu command for key '{key}'. User-defined max hotend temp '{hotend_max}' exceeded!")
						else:
	...
	def on_settings_migrate(self, target, current=None):
		...
		if current < 1:
			settings = traverse_modify(settings, ["profiles", [], "value", "commands", [], "value", "pressed", []],
									   migrate_gcode_to_v1)
			settings = traverse_modify(settings, ["profiles", [], "value", "commands", [], "value", "released", []],
									   migrate_gcode_to_v1)
			...
			profiles=[
				...
				{"key": "Example 10key - evdev", "value": {
					...
					"commands": [
						# **************************** Linux ******************************
						{"key": "kpdot", "alias": ".",
						 "value": {"pressed": [{"type": "listen_vars", "variables": ["distance", "hotend", "bed"]}],
								   "released": [
									   {"type": "octoprint", "command": "confirm_last_command", "presses_required": 1},
									   {"type": "save_vars", "variables": ["distance", "hotend", "bed"]}],
								   "variables": []}},  # making this my variable modifier
						{"key": "kpenter", "alias": "enter",
						 "value": {"pressed": [{"type": "printer", "gcode": ["G28 Z"], "send_while_printing": False}],
								   "released": [], "variables": []}},  # homing z
						{"key": "kp0", "alias": "0",
						 "value": {"pressed": [{"type": "printer", "gcode": ["G28 X Y"], "send_while_printing": False}],
								   "released": [], "variables": [{"key": "distance", "value": 0.1}]}},  # homing x, y
						{"key": "kp1", "alias": "1", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X10 Y10 F6000"], "send_while_printing": False}],
															   "released": [],
															   "variables": [{"key": "distance", "value": 1}]}},
						# front left corner, 10x10 in
						{"key": "kp2", "alias": "2", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Y-<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															   "variables": [{"key": "distance", "value": 10}]}},
						# move south
						{"key": "kp3", "alias": "3", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X290 Y10 F6000"], "send_while_printing": False}],
															   "released": [],
															   "variables": [{"key": "distance", "value": 100}]}},
						# front right corner, 10x10 in
						{"key": "kp4", "alias": "4", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 X-<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															   "variables": [{"key": "bed", "value": 0}]}},  # move west
						{"key": "kp5", "alias": "5", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X150 Y150 F6000"], "send_while_printing": False}],
															   "released": [],
															   "variables": [{"key": "bed", "value": 50}]}},  # center
						{"key": "kp6", "alias": "6", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 X+<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															   "variables": [{"key": "bed", "value": 60}]}},
						# move east
						{"key": "kp7", "alias": "7", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X10 Y290 F6000"], "send_while_printing": False}],
															   "released": [],
															   "variables": [{"key": "hotend", "value": 0}]}},
						# rear left corner, 10x10 in
						{"key": "kp8", "alias": "8", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Y+<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															   "variables": [{"key": "hotend", "value": 205}]}},
						# move north
						{"key": "kp9", "alias": "9", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X290 Y290 F6000"], "send_while_printing": False}],
															   "released": [],
															   "variables": [{"key": "hotend", "value": 210}]}},
						# rear right corner, 10x10 in
						{"key": "kpplus", "alias": "+", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Z-<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move down
						{"key": "kpminus", "alias": "-", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Z+<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move up
						{"key": "kpasterisk", "alias": "*", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G1 E-<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move up
						{"key": "backspace", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G1 E+<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move up
						{"key": "kpslash", "alias": "/", "value": {"pressed": [
							{"type": "printer", "gcode": ["M104 S<hotend>", "M140 S<bed>"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # set hotend and bed
						{"key": "tab", "alias": None,
						 "value": {"pressed": [{"type": "octoprint", "command": "start_print", "presses_required": 5}],
								   "released": [], "variables": []}},  # start selected print
						{"key": "equal", "alias": "=",
						 "value": {"pressed": [{"type": "octoprint", "command": "cancel_print", "presses_required": 5}],
								   "released": [], "variables": []}},  # stop running print
						{"key": "esc", "alias": None,
						 "value": {"pressed": [{"type": "plugin_psucontrol", "command": "toggle", "hotend_max": 50}],
								   "released": [], "variables": []}},  # turn off PSU if hotends < 50c
					],
					"keyboard": {
						"scale": 3,
						"board": [
							{"keys": [{"key": "esc", "alias": None, "w": 1, "h": 1},
									  {"key": None, "alias": None, "w": 1, "h": 1},
									  {"key": "tab", "alias": None, "w": 1, "h": 1},
									  {"key": "equal", "alias": "=", "w": 1, "h": 1}]},
							{"keys": [{"key": None, "alias": None, "w": 1, "h": 1},
									  {"key": "kpslash", "alias": "/", "w": 1, "h": 1},
									  {"key": "kpasterisk", "alias": "*", "w": 1, "h": 1},
									  {"key": "backspace", "alias": None, "w": 1, "h": 1}]},
							{"keys": [{"key": "kp7", "alias": "7", "w": 1, "h": 1},
									  {"key": "kp8", "alias": "8", "w": 1, "h": 1},
									  {"key": "kp9", "alias": "9", "w": 1, "h": 1},
									  {"key": "kpminus", "alias": "-", "w": 1, "h": 1}]},
							{"keys": [{"key": "kp4", "alias": "4", "w": 1, "h": 1},
									  {"key": "kp5", "alias": "5", "w": 1, "h": 1},
									  {"key": "kp6", "alias": "6", "w": 1, "h": 1},
									  {"key": "kpplus", "alias": "+", "w": 1, "h": 1}]},
							{"keys": [{"key": "kp1", "alias": "1", "w": 1, "h": 1},
									  {"key": "kp2", "alias": "2", "w": 1, "h": 1},
									  {"key": "kp3", "alias": "3", "w": 1, "h": 1},
									  {"key": "kpenter", "alias": "enter", "w": 1, "h": 2}]},
							{"keys": [{"key": "kp0", "alias": "0", "w": 2, "h": 1},
									  {"key": "kpdot", "alias": ".", "w": 1, "h": 1}]}
						]
					},
					"variables": [
				...
				{"key": "Example 10key - pynput", "value": {
					...
					"commands": [
						# **************************** Mac ******************************
						{"key": ".", "alias": None, "value": {"pressed": [
							{"type": "listen_vars", "variables": ["distance", "hotend", "bed"],
							 "send_while_printing": False}], "released": [
							{"type": "octoprint", "command": "confirm_last_command", "presses_required": 1},
							{"type": "save_vars", "variables": ["distance", "hotend", "bed"]}], "variables": []}},
						...
						{"key": "0", "alias": None,
						 "value": {"pressed": [{"type": "printer", "gcode": ["G28 X Y"], "send_while_printing": False}],
								   "released": [], "variables": [{"key": "distance", "value": 0.1}]}},  # homing x, y
						{"key": "1", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X10 Y10 F6000"], "send_while_printing": False}],
															  "released": [],
															  "variables": [{"key": "distance", "value": 1}]}},
						# front left corner, 10x10 in
						{"key": "2", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Y-<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															  "variables": [{"key": "distance", "value": 10}]}},
						# move south
						{"key": "3", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X290 Y10 F6000"], "send_while_printing": False}],
															  "released": [],
															  "variables": [{"key": "distance", "value": 100}]}},
						# front right corner, 10x10 in
						{"key": "4", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 X-<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															  "variables": [{"key": "bed", "value": 0}]}},  # move west
						{"key": "5", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X150 Y150 F6000"], "send_while_printing": False}],
															  "released": [],
															  "variables": [{"key": "bed", "value": 50}]}},  # center
						{"key": "6", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 X+<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															  "variables": [{"key": "bed", "value": 60}]}},  # move east
						{"key": "7", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X10 Y290 F6000"], "send_while_printing": False}],
															  "released": [],
															  "variables": [{"key": "hotend", "value": 0}]}},
						# rear left corner, 10x10 in
						{"key": "8", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Y+<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															  "variables": [{"key": "hotend", "value": 205}]}},
						# move north
						{"key": "9", "alias": None, "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X290 Y290 F6000"], "send_while_printing": False}],
															  "released": [],
															  "variables": [{"key": "hotend", "value": 210}]}},
						# rear right corner, 10x10 in
					...
					"keyboard": {
						...
						"board": [
							...
							{"keys": [{"key": None, "alias": None, "w": 1, "h": 1},
									  ...
									  {"key": "*", "alias": None, "w": 1, "h": 1},
									  {"key": "backspace", "alias": None, "w": 1, "h": 1}]},
							{"keys": [{"key": "7", "alias": None, "w": 1, "h": 1},
									  {"key": "8", "alias": None, "w": 1, "h": 1},
									  {"key": "9", "alias": None, "w": 1, "h": 1},
									  {"key": "-", "alias": None, "w": 1, "h": 1}]},
							{"keys": [{"key": "4", "alias": None, "w": 1, "h": 1},
									  {"key": "5", "alias": None, "w": 1, "h": 1},
									  {"key": "6", "alias": None, "w": 1, "h": 1},
									  {"key": "+", "alias": None, "w": 1, "h": 1}]},
							{"keys": [{"key": "1", "alias": None, "w": 1, "h": 1},
									  {"key": "2", "alias": None, "w": 1, "h": 1},
									  {"key": "3", "alias": None, "w": 1, "h": 1},
									  {"key": "\\x03", "alias": "enter", "w": 1, "h": 2}]},
							{"keys": [{"key": "0", "alias": None, "w": 2, "h": 1},
									  {"key": ".", "alias": None, "w": 1, "h": 1}]}
						]
					},
					"variables": [
				...
				{"key": "Example 10key - pynput with unicode symbols", "value": {
					...
					"commands": [
						...
						{"key": "0", "alias": "⟰ XY",
						 "value": {"pressed": [{"type": "printer", "gcode": ["G28 X Y"], "send_while_printing": False}],
								   "released": [], "variables": [{"key": "distance", "value": 0.1}]}},  # homing x, y
						{"key": "1", "alias": "⬋", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X10 Y10 F6000"], "send_while_printing": False}],
															 "released": [],
															 "variables": [{"key": "distance", "value": 1}]}},
						# front left corner, 10x10 in
						{"key": "2", "alias": "▼", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Y-<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															 "variables": [{"key": "distance", "value": 10}]}},
						# move south
						{"key": "3", "alias": "⬊", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X290 Y10 F6000"], "send_while_printing": False}],
															 "released": [],
															 "variables": [{"key": "distance", "value": 100}]}},
						# front right corner, 10x10 in
						{"key": "4", "alias": "◀︎", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 X-<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															  "variables": [{"key": "bed", "value": 0}]}},  # move west
						{"key": "5", "alias": "●", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X150 Y150 F6000"], "send_while_printing": False}],
															 "released": [],
															 "variables": [{"key": "bed", "value": 50}]}},  # center
						{"key": "6", "alias": "▶︎", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 X+<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															  "variables": [{"key": "bed", "value": 60}]}},  # move east
						{"key": "7", "alias": "⬉", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X10 Y290 F6000"], "send_while_printing": False}],
															 "released": [],
															 "variables": [{"key": "hotend", "value": 0}]}},
						# rear left corner, 10x10 in
						{"key": "8", "alias": "▲", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Y+<distance> F6000", "G90"],
							 "send_while_printing": False}], "released": [],
															 "variables": [{"key": "hotend", "value": 205}]}},
						# move north
						{"key": "9", "alias": "⬈", "value": {"pressed": [
							{"type": "printer", "gcode": ["G0 X290 Y290 F6000"], "send_while_printing": False}],
															 "released": [],
															 "variables": [{"key": "hotend", "value": 210}]}},
						# rear right corner, 10x10 in
						{"key": "+", "alias": "⬇︎", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Z-<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move down
						{"key": "-", "alias": "⬆︎", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G0 Z+<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move up
						{"key": "*", "alias": "E-", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G1 E-<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move up
						{"key": "backspace", "alias": "E+", "value": {"pressed": [
							{"type": "printer", "gcode": ["G91", "G1 E+<distance> F300", "G90"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # move up
						{"key": "/", "alias": "°C", "value": {"pressed": [
							{"type": "printer", "gcode": ["M104 S<hotend>", "M140 S<bed>"],
							 "send_while_printing": False}], "released": [], "variables": []}},  # set hotend and bed
						{"key": "tab", "alias": ">",
						 "value": {"pressed": [{"type": "octoprint", "command": "start_print", "presses_required": 5}],
					...
					"keyboard": {
						"scale": 3,
						"board": [
							{"keys": [{"key": "esc", "alias": "PSU", "w": 1, "h": 1},
									  {"key": None, "alias": None, "w": 1, "h": 1},
									  {"key": "tab", "alias": ">", "w": 1, "h": 1},
									  {"key": "=", "alias": "⬣", "w": 1, "h": 1}]},
							{"keys": [{"key": None, "alias": None, "w": 1, "h": 1},
									  {"key": "/", "alias": "°C", "w": 1, "h": 1},
									  {"key": "*", "alias": "E-", "w": 1, "h": 1},
									  {"key": "backspace", "alias": "E+", "w": 1, "h": 1}]},
							{"keys": [{"key": "7", "alias": "⬉", "w": 1, "h": 1},
									  {"key": "8", "alias": "▲", "w": 1, "h": 1},
									  {"key": "9", "alias": "⬈", "w": 1, "h": 1},
									  {"key": "-", "alias": "⬆︎", "w": 1, "h": 1}]},
							{"keys": [{"key": "4", "alias": "◀︎", "w": 1, "h": 1},
									  {"key": "5", "alias": "●", "w": 1, "h": 1},
									  {"key": "6", "alias": "▶︎", "w": 1, "h": 1},
									  {"key": "+", "alias": "⬇︎", "w": 1, "h": 1}]},
							{"keys": [{"key": "1", "alias": "⬋", "w": 1, "h": 1},
									  {"key": "2", "alias": "▼", "w": 1, "h": 1},
									  {"key": "3", "alias": "⬊", "w": 1, "h": 1},
									  {"key": "\\x03", "alias": "⟰ Z", "w": 1, "h": 2}]},
							{"keys": [{"key": "0", "alias": "⟰ XY", "w": 2, "h": 1},
									  {"key": ".", "alias": "❖", "w": 1, "h": 1}]}
						]
					},
					"variables": [
